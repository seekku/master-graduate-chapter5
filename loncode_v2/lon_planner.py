...
def run(frames=1000, eps_fixed=False, eps_frames=1e6, min_eps=0.01,collision = 0,success =0,writer = None):
    ...
    for frame in range(1, frames + 1):
        ...
        latepoch_reward += reward
        latepoch_lat_reward += lat_reward
        obs = obs2
        obs2 = obs3
        ...
        if done:
            ...
            print("this epoch lat_reward",total_lat_reward)
...
writer = SummaryWriter("runs/" + "lon_v2-FQF100")
seed = 100
BUFFER_SIZE = 40000
BATCH_SIZE = 64
GAMMA = 0.99
...
t0 = time.time()
final_average100 = run(frames = 100000, eps_fixed=eps_fixed, eps_frames=500, min_eps=0.05, collision = 0,writer=writer)
t1 = time.time()

print("Training time: {}min".format(round((t1-t0)/60,2)))
torch.save(agent.qnetwork_local.state_dict(), "lon_v2_FQF-QNetseed100"+".pth")
torch.save(agent.FPN.state_dict(),"lon_v2_FQF-FPNseed100"+".pth")
